...
namespace HHotelSearchAPI.Common.Services
{
    public class AuthManager : IAuthManager
    {
        ...
        {
            await _userManager.RemoveAuthenticationTokenAsync(_user, "HotelSearchAPI", "RefreshToken");
            var newRefreshToken = await _userManager.GenerateUserTokenAsync(_user, "HotelSearchAPI", "RefreshToken");
            var result = await _userManager.SetAuthenticationTokenAsync(_user, "HotelSearchAPI", "RefreshToken", newRefreshToken);
        ...
        public async Task<TokenRequest> VerifyRefreshToken(TokenRequest request)
        {
            ...
            try
            {
                var isValid = await _userManager.VerifyUserTokenAsync(_user, "HotelSearchAPI", "RefreshToken", request.RefreshToken);
                if (isValid)
                {
